...
# A code snippet is valid if all the following rules hold: The code must be wrapped in a valid closed tag .
...
# Example 1: Input: code = "<DIV>This is the first line <![CDATA[<div>]]></DIV>" Output: true Explanation: The code is wrapped in a closed tag : <DIV> and </DIV>.
# The TAG_NAME is valid, the TAG_CONTENT consists of some characters and cdata.
# Although CDATA_CONTENT has an unmatched start tag with invalid TAG_NAME, it should be considered as plain text, not parsed as a tag.
# So TAG_CONTENT is valid, and then the code is valid.
# Thus return true.
# Example 2: Input: code = "<DIV>>>  ![cdata[]] <![CDATA[<div>]>]]>]]>>]</DIV>" Output: true Explanation: We first separate the code into : start_tag|tag_content|end_tag.
...
# Example 3: Input: code = "<A>  <B> </A>   </B>" Output: false Explanation: Unbalanced.
# If "<A>" is closed, then "<B>" must be unmatched, and vice versa.
# Constraints: 1 <= code.length <= 500 code consists of English letters, digits, '<' , '>' , '/' , '!' , '[' , ']' , '.' , and ' ' .
