...
# Implement the RLEIterator class: RLEIterator(int[] encoded) Initializes the object with the encoded array encoded . int next(int n) Exhausts the next n elements and returns the last element exhausted in this way.
# If there is no element left to exhaust, return -1 instead.
# Example 1: Input ["RLEIterator", "next", "next", "next", "next"]
# [[[3, 8, 0, 9, 2, 5]], [2], [1], [1], [2]] Output [null, 8, 8, 5, -1] Explanation RLEIterator rLEIterator = new RLEIterator([3, 8, 0, 9, 2, 5]); // This maps to the sequence [8,8,8,5,5].
...
# rLEIterator.next(2); // exhausts 2 terms, returning -1.
...
# Constraints: 2 <= encoding.length <= 1000 encoding.length is even. 0 <= encoding[i] <= 10 9 1 <= n <= 10 9 At most 1000 calls will be made to next .
