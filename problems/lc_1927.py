...
# On each turn, a player will do the following if there is still at least one '?' in num : Choose an index i where num[i] == '?' .
...
# Example 1: Input: num = "5023" Output: false Explanation: There are no moves to be made.
...
# Example 3: Input: num = "?3295???" Output: false Explanation: It can be proven that Bob will always win.
...
# Constraints: 2 <= num.length <= 10 5 num.length is even . num consists of only digits and '?' .
