...
# You are given a 2D integer array orders , where each orders[i] = [price i , amount i , orderType i ] denotes that amount i orders have been placed of type orderType i at the price price i .
# The orderType i is: 0 if it is a batch of buy orders, or 1 if it is a batch of sell orders.
...
# When an order is placed, the following happens: If the order is a buy order, you look at the sell order with the smallest price in the backlog.
...
# Example 1: Input: orders = [[10,5,0],[15,2,1],[25,1,1],[30,4,0]] Output: 6 Explanation: Here is what happens with the orders:
...
# The 3 rd order is matched with the sell order of the least price, which is 25 and this sell order is removed from the backlog.
# Then, there are no more sell orders in the backlog, so the 4 th order is added to the backlog.
...
# Example 2: Input: orders = [[7,1000000000,1],[15,3,0],[5,999999995,0],[5,1,1]] Output: 999999984 Explanation: Here is what happens with the orders:
# - 10 9 orders of type sell with price 7 are placed.
# There are no buy orders, so the 10 9 orders are added to the backlog.
...
# Constraints: 1 <= orders.length <= 10 5 orders[i].length == 3 1 <= price i , amount i <= 10 9 orderType i is either 0 or 1 .
