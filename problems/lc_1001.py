...
# You are given a 2D array of lamp positions lamps , where lamps[i] = [row i , col i ] indicates that the lamp at grid[row i ][col i ] is turned on .
...
# Example 1: Input: n = 5, lamps = [[0,0],[4,4]], queries = [[1,1],[1,0]] Output: [1,0] Explanation: We have the initial grid with all lamps turned off.
# In the above picture we see the grid after turning on the lamp at grid[0][0] then turning on the lamp at grid[4][4].
# The 0 th query asks if the lamp at grid[1][1] is illuminated or not (the blue square).
# It is illuminated, so set ans[0] = 1.
# Then, we turn off all lamps in the red square.
# The 1 st query asks if the lamp at grid[1][0] is illuminated or not (the blue square).
# It is not illuminated, so set ans[1] = 0.
# Then, we turn off all lamps in the red rectangle.
# Example 2: Input: n = 5, lamps = [[0,0],[4,4]], queries = [[1,1],[1,1]] Output: [1,1]
# Example 3: Input: n = 5, lamps = [[0,0],[0,4]], queries = [[0,4],[0,1],[1,4]] Output: [1,1,0] Constraints: 1 <= n <= 10 9 0 <= lamps.length <= 20000 0 <= queries.length <= 20000 lamps[i].length == 2 0 <= row i , col i < n queries[j].length == 2 0 <= row j , col j < n
