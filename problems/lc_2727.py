...
# Example 3: Input: obj = [null, false, 0] Output: false Explanation: The array has 3 elements so it is not empty.
# Constraints: obj is a valid JSON object or array 2 <= JSON.stringify(obj).length <= 10 5 Can you solve it in O(1) time?

# This question has no Python code stub.
# Generating a generic Python code stub
class Solution:
    def isObjectEmpty(self) -> Any:
      return
