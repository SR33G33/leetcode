...
# Example 1: Input: colors = "AAABABB" Output: true Explanation: A A ABABB -> AABABB
# Alice moves first.
# She removes the second 'A' from the left since that is the only 'A' whose neighbors are both 'A'.
# Now it's Bob's turn.
# Bob cannot make a move on his turn since there are no 'B's whose neighbors are both 'B'.
# Thus, Alice wins, so return true.
# Example 2: Input: colors = "AA" Output: false Explanation: Alice has her turn first.
# There are only two 'A's and both are on the edge of the line, so she cannot move on her turn.
# Thus, Bob wins, so return false.
# Example 3: Input: colors = "ABBBBBBBAAA" Output: false Explanation: ABBBBBBBA A A -> ABBBBBBBAA
# Alice moves first.
...
# Thus, Bob wins, so return false.
# Constraints: 1 <= colors.length <= 10 5 colors consists of only the letters 'A' and 'B'
