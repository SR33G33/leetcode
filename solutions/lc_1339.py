...
# Example 2: Input: root = [1,None,2,3,4,None,None,5,6] Output: 90 Explanation: Remove the red edge and get 2 binary trees with sum 15 and 6.Their product is 90 (15*6) Constraints: The number of nodes in the tree is in the range [2, 5 * 10 4 ] . 1 <= Node.val <= 10 4
...
class Solution:
    nodeTable = {None : 0}
    def maxProduct(self, root, total=None) -> int:
        def dfsHelper(node):
            if not node: return 0
            treeTotal = node.val + dfsHelper(node.left) + dfsHelper(node.right)
            self.nodeTable[node] = treeTotal 
            # print(self.nodeTable)
            return treeTotal

        currentMax = 0
        topRoot = False
        if(root is None): return currentMax

        # print(F"{root.val}, currentMax: {currentMax}, left_sum: {dfsHelper(root.left)}")

        if(total is None):
            topRoot = True
            total = dfsHelper(root)

        # print(f"left node of {root.val}, {root.left}")
        # print(self.nodeTable)
        leftTotal = self.nodeTable[root.left]
        rightTotal = self.nodeTable[root.right]

        currentMax = max(currentMax, leftTotal*(total-leftTotal), rightTotal*(total-rightTotal), self.maxProduct(root.left, total), self.maxProduct(root.right, total))

        if(topRoot is True):
            return currentMax % (10**9 + 7)
        return currentMax
